                    ...
                    <div class="mt-4 overflow-hidden block">
                        ...
                        <div class="flex justify-between sm:w-[160px] md:w-[200px]">
                            <div class="text-[#1d242d]">
                                <a :href="`/package/${item.id}/${slug(item.name)}`">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            top10(data){
                let packages = [...data]
                const count = Math.min(10, packages.length);
                this.data = Array.from({ length: count }, () => packages.splice(Math.floor(Math.random() * packages.length), 1)[0])
                this.ready = true
            },
            ...
            addToCart(id){
                ...
                let item = {
                    id : module.id,
                    type : 'package',
                    image : module.relationships.modules[0].images[0].url,
                    price : module.price,
                    name : module.name
                }
                this.addLocal(item)
